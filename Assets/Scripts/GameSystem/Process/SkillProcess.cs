...
namespace SkillProcess
{
    public static class SkillCast
    {
        ...
        {
            Action<IDefender> activation = target =>
            {
        ...
        private static void AnimationProcess(IStateMachine attacker, ICastAnimationSkill skill, Action activation)
        {
    ...
    public static class SkillActivate
    {
        ...
        // HP 감소 계산
        // 사망 판정
        private static void DamagingProcess(IAttacker Attacker, IDefender Defender, IDamagingSkill DamagingSkill)
        {
            Debug.Log("DamagingProcess");
            //Defender.CurrentHP -= Attacker.GetProperty<ATK_BattleProperty>() * DamagingSkill.SkillDamageRatio;
        }

        private static void DebuffProcess(IAttacker Attacker, IDefender Defender, IDebuffSkill DebuffSkill)
        {
            Debug.Log("DebuffProcess");
        }

        private static void BuffProcess(IAttacker Attacker, IDefender Defender, IBuffSkill BuffSkill)
        {
