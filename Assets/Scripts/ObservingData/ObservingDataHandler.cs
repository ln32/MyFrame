...
{
    // 타입별 딕셔너리 1:1대응
    private readonly Dictionary<Type, iDictionaryProvider> typeDictionaryMap = new();
    ...
    {
        var target = PairObj.DataNameList;
        if (!typeDictionaryMap.ContainsKey(typeof(T))) typeDictionaryMap[typeof(T)] = new TypeDictionary<T>();

        for (var i = 0; i < target.Count; i++)
        {
            var valueName = target[i];
    ...
    {
        var type = typeof(T);
        if (!typeDictionaryMap.ContainsKey(type)) typeDictionaryMap[type] = new TypeDictionary<T>();
    }
